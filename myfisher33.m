...
p = inputParser;
addRequired(p,'x',@(x) validateattributes(x,{'numeric'},{'real','finite','integer','nonnegative','nonnan','size',[3 3]}));
parse(p,x);
clear p
...
disp(tr)
disp(array2table([et2,P,0.5*op+sum(np(np<op))],'VariableNames',{'Tables','two_tails_p_value','Mid_p_correction'}));
